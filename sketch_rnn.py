...
class EncoderRNN(nn.Module):
    ...
    def forward(self, inputs, batch_size, hidden_cell=None):
        ...
        hidden_forward, hidden_backward = torch.split(
            self.dropout(hidden), 1, 0)
    ...
    def forward(self, inputs, z, hidden_cell=None):
        if hidden_cell is None:
            ...
            hidden, cell = torch.split(
                torch.tanh(self.fc_hc(z)), hp.dec_hidden_size, 1)
        ...
        if self.training:
            ...
            y = self.fc_params(self.dropout(
                hidden).view(-1, hp.dec_hidden_size))
...
class SketchRNN(nn.Module):
    ...
    def make_target(self, batch, lengths):
        if use_cuda:
            eos = torch.stack([torch.Tensor([0, 0, 0, 0, 1])]
    ...
    def sample_next_state(self):
        ...
        x, y = sample_bivariate_normal(
            mu_x, mu_y, sigma_x, sigma_y, rho_xy, greedy=False)
